...
class PostgresMaterialStore:
    ...
    def list_extraction_runs(self, material_id: str) -> list[dict[str, Any]]:
        """Return extraction run history for one material, newest first."""
        material_id = _require_material_id(material_id)
        with self._connection() as connection:
            rows = connection.execute(
                """
                SELECT extraction_run_id::text, material_id, status, provider,
                       source_format, structured_records_path, source_id,
                       extractor_name, extractor_version, record_count,
                       error_message, started_at, completed_at, parameters,
                       result_summary
    ...
    def list_extraction_artifacts(self, material_id: str) -> list[dict[str, Any]]:
        """Return extraction artifact references for one material, newest first."""
        material_id = _require_material_id(material_id)
        with self._connection() as connection:
            rows = connection.execute(
                """
        ...
        return [_extraction_artifact_from_row(row) for row in rows]
...
def _extraction_artifact_from_row(row: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "artifact_id": str(row["artifact_id"]),
        "material_id": row["material_id"],
        "extraction_run_id": (
            str(row["extraction_run_id"]) if row.get("extraction_run_id") else None
        ),

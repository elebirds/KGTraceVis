...
def dashboard_fields_from_analysis(
    ...
    linked_entities, top_k_paths, ranked_root_causes, _changed = (
...
def normalize_case_reasoning_payload(
    ...
    *,
    ...
    """Backfill stable evidence-support IDs and display fields for dashboard rows."""
        ...
        if not link.get("selected_entity_name"):
            ...
            selected_name = next(
                (
                    ...
                    if candidate.get("entity_id") == selected_entity_id
    ...
    for path in updated_paths:
        ...
        existing_support_ids = _read_string_array(path.get("support_obs_ids"))
        if support_obs_ids and support_obs_ids != existing_support_ids:
...
def _enrich_case_row(
    ...
    *,
    ...
    if evidence and row.get("generated_evidence") != evidence:
        ...
        changed = True
    case_id = str(row.get("case_id") or evidence.get("case_id") or "unknown")
...
def _default_case_rows(
    ...
    *,
    ...
    case_id = str(evidence.get("case_id") or case_summary.get("case_id") or "unknown")
    ...
    return [
        {
            "case_id": case_id,
            "case_label": evidence.get("case_label")

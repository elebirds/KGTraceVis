"""Workflow for deterministic noise reproducibility experiments."""
...
class NoiseExperimentPipeline(Protocol):
    """Pipeline interface required by the noise experiment workflow."""

    def analyze(self, evidence: Evidence) -> AnalysisResult:
        """Analyze one evidence item."""


@dataclass(frozen=True)
class NoiseExperimentConfig:
    """Configuration paths for the noise experiment workflow."""

    noise_config: Path = Path("configs/noise_config.yaml")
    experiment_config: Path = Path("configs/experiment_config.yaml")


@dataclass(frozen=True)
class NoiseExperimentResult:
    """Result of a deterministic noise experiment workflow run."""
...
def run_noise_experiment(
    ...
    *,
    ...
    config = config or NoiseExperimentConfig()
    noise_config = _read_yaml(config.noise_config)
    experiment_config = _read_yaml(config.experiment_config)
    ...
    analysis_pipeline = pipeline or KGTracePipeline()
    clean_results = {
        item.case_id: analysis_pipeline.analyze(item) for item in evidence_items
    }
    ...
    for noise_level in noise_levels:
        for noise_type in noise_types:
            for evidence in evidence_items:
                ...
                noisy_result = analysis_pipeline.analyze(noisy)
...
def _linking_inputs(
    ...
    candidates = [list(noisy_by_field.get(field, {}).get("candidates", [])) for field in fields]

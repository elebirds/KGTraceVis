...
uv run python scripts/build_wafer_llm_source_pack.py --output-dir /tmp/kgtracevis_wafer_source_pack_expanded_papers_v4 --exclude-wm811k-records --overwrite
uv run python scripts/smoke_wafer_llm_kg_construction.py --source-pack /tmp/kgtracevis_wafer_source_pack_expanded_papers_v4/wafer_llm_source_pack.json --output-dir /tmp/kgtracevis_wafer_expanded_papers_live_5src_raw_v4 --provider openai --max-materials 5 --document-understanding-mode long_context --max-chars 12000 --overlap-chars 1200 --overwrite
...
A later wafer paper-only live pass excluded local WM811K example records and
used five external web/PDF sources through the same material pipeline. It
produced `55` nodes, `22` edges, `5` document maps, `12` brainstorm hypotheses,
and `13` brainstorm review items, again with `0` published edges. Material
extraction persisted `document_ie_raw_responses.jsonl` before
`document_ie_payload_repairs.jsonl`, so schema repair and ID normalization can
be audited from saved model output instead of regenerated by another LLM call.
